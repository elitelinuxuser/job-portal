...
export function BookingPaymentHistory({ payments, booking }: BookingPaymentHistoryProps) {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
            ...
            >
              <div className="flex justify-between items-start mb-2">
                <div>
                  ...
                  {payment.status === 'disputed' && payment.updatedAt && (
                    <p className="text-xs text-red-600 mt-1">
                      Dispute raised on {format(new Date(payment.updatedAt), 'MMM d, yyyy')}
                    </p>
                  )}
                  {payment.paymentMode && (
                    <p className="text-xs text-gray-600 mt-1">
