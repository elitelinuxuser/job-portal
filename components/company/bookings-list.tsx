        ...
        <CardContent>
          <div className="py-16 text-center">
            ...
            <p className="text-gray-600">
              Send booking requests to freelancers to get started
            </p>
          </div>
        </CardContent>
      </Card>
      ...
      {currentBookings.length === 0 ? (
        ...
        <div className="grid gap-4">
          ...
          {currentBookings.map((booking: any) => {
            ...
            return (
              <Link key={booking.id} href={`/company/bookings/${booking.id}`}>
                <Card className="overflow-hidden hover:shadow-lg transition-all duration-200 border-2 cursor-pointer hover:border-indigo-300">
                  <CardContent className="p-0">
                    <div className="px-4">
                      {/* Header - Freelancer and Amount */}
                      <div className="flex items-start gap-3 mb-2">
                        ...
                        {freelancerProfile?.photoUrl ? (
                          ...
                          <div className="w-10 h-10 rounded-full bg-linear-to-br from-indigo-500 to-purple-600 flex items-center justify-center text-white font-semibold shrink-0 text-sm">
                            {(freelancerProfile?.name || 'F').charAt(0).toUpperCase()}
                          </div>
                        )}
