...
export interface CompanyPaymentFilterState {
  search: string
  statuses: string[]
  minAmount: string
  maxAmount: string
...
interface CompanyPaymentFiltersProps {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  payments: any[]
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  onFilterChange: (filters: CompanyPaymentFilterState, filteredPayments: any[]) => void
}
...
export function CompanyPaymentFilters({ payments, onFilterChange }: CompanyPaymentFiltersProps) {
  const [isOpen, setIsOpen] = useState(false)
  const [filters, setFilters] = useState<CompanyPaymentFilterState>({
    search: '',
    statuses: [],
    minAmount: '',
    maxAmount: '',
  ...
  const filteredPayments = useMemo(() => {
    ...
    if (filters.search) {
      ...
      filtered = filtered.filter(payment => 
        ...
        payment.booking?.freelancer?.freelancerProfile?.name?.toLowerCase().includes(searchLower) ||
  ...
  const clearAllFilters = () => {
    const initialFilters: CompanyPaymentFilterState = {
      search: '',
      statuses: [],
      minAmount: '',
      maxAmount: '',
    ...
    <div className="space-y-3">
      ...
      <div className="flex gap-3">
        <div className="flex-1 relative">
          ...
          <Input
            type="text"
            placeholder="Search payments, freelancers, or notes..."
